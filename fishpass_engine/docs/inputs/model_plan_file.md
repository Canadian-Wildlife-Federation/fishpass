...
| structure_types | Required. List of barrier types to include (`dams`, `waterfalls`, `stream_crossings`, `gradients`, `beaver_dams`, etc.). Pulled from the CABD API and the new_structures table. `gradients` must be in the list to include graident barriers in the analysis otherwise they will be excluded. |
