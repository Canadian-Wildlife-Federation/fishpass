...
| reporting_values | Required. List of species/lifecycle combinations required in outputs, encoded as `<species>_<lifecycle>`. Valid lifecycles: `rear`, `spawn`, `general`. `all` can be used in either position to mean all target species, or all lifecycles. Examples: `as_rear`, `as_spawn`, `as_general`, `all_rear`, `all_spawn`, `all_general`, `all_all` |
