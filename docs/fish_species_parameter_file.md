...
| strahler\_order\_spawning\_min | Used when assigning habitat. If there is no spawning habitat for this species then set the max \< min. |
| strahler\_order\_spawning\_max | Used when assigning habitat. If there is no spawning habitat for this species then set the max \< min. |
| strahler\_order\_rearing\_min | Used when assigning habitat. If there is no rearing habitat for this species then set the max \< min. |
| strahler\_order\_rearing\_max | Used when assigning habitat. If there is no rearing habitat for this species then set the max \< min. |
...
| fall\_height\_threshold | **Not currently used.** The height of natural barriers (waterfalls) in meters at which a given species can't move upstream (i.e. becomes impassible) |
| spawn\_discharge\_min | **Not currently used.** Minimum stream discharge for suitable spawning habitat for the species. Only populated for where discharge data is available for streams |
| spawn\_discharge\_max |**Not currently used.** Maximum stream discharge for suitable spawning habitat for the species. Only populated for where discharge data is available for streams |
| rear\_discharge\_min | **Not currently used.** Minimum stream discharge for suitable rearing habitat for the species. Only populated for where discharge data is available for streams |
| rear\_discharge\_max | **Not currently used.** Maximum stream discharge for suitable rearing habitat for the species. Only populated for where discharge data is available for streams |
| spawn\_channel\_confinement\_min | **Not currently used.** Minimum channel confinement for suitable spawning habitat for the species. Only populated for where channel confinement data is available for streams  |
| spawn\_channel\_confinement\_max | **Not currently used.** Maximum channel confinement for suitable spawning habitat for the species. Only populated for where channel confinement data is available for streams  |
| rear\_channel\_confinement\_min | **Not currently used.** Minimum channel confinement for suitable rearing habitat for the species. Only populated for where channel confinement data is available for streams |
| rear\_channel\_confinement\_max | **Not currently used.** Maximum channel confinement for suitable rearing habitat for the species. Only populated for where channel confinement data is available for streams  |
